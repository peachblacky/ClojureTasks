...
(defn append-sequence
  ...
  (if (empty? alphabet)
    ...
    (let [new-word (append-char sequence (first alphabet))
      ...
      (if (nil? new-word)
        (append-sequence sequence new-seq seq-acc)
        (append-sequence sequence new-seq (into seq-acc (list new-word)))))))
...
(defn append-sequences
  ...
  (if (empty? sequences)
    seq-acc
    (append-sequences (rest sequences) alphabet (into seq-acc (append-sequence (first sequences) alphabet [])))))
  ...
  (if (empty? alphabet)
    ...
    (if (empty? seq-acc)
      ...
      (if (= (count (first seq-acc)) n)
        seq-acc
        (solve (append-sequences seq-acc alphabet []) alphabet n)))))
